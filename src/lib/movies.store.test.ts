...
describe('Movies Store (Svelte 5 Runes)', () => {
  ...
  describe('toggleFavorite()', () => {
    it('debería actualizar el estado de favorito de una película en el store', async () => {
      vi.mocked(api.getMovies).mockResolvedValue([...mockMovies]);
      await moviesStore.loadMovies();

      const toggledMovie = { ...mockMovies[0], isFavorite: true };
    ...
    it('debería manejar error al alternar favorito', async () => {
      vi.mocked(api.toggleFavorite).mockRejectedValue(new Error('API Exception'));

      const ok = await moviesStore.toggleFavorite('999');

      expect(ok).toBe(false);
      expect(moviesStore.error).toBe('API Exception');
      expect(moviesStore.mutating).toBe(false);
    });
  });
});
